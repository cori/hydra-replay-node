    ...
    for (const commandName of commandNames) {
      ...
      if (hk.enabled && typeof commands[commandName] === "function") {
        hotkeys(hk.key, function (e, hotkeyHandler) {
          ...
          handler({ key: hk.key, name: commandName, cursor: cm.getCursor() });
        });
      }
    }
  }
  exec(cm, activity) {
    if (commands[activity.name] !== undefined) {
      if (activity.cursor !== undefined) {
        cm.focus();
        cm.setCursor(activity.cursor);
      }
      commands[activity.name]({ cm, emitter: this.emitter });
    }
  }
}
